...
def datingClassTest2():
    hoRatio = 0.5
    datingDataMat, datingLabels = KNN2.file2matrix('datingTestSet2.txt')  # load data setfrom file
    normMat, ranges, minVals = autoNorm(datingDataMat)
    m = normMat.shape[0]
    numTestMat = int(hoRatio * m)
    erroCount = 0
    for i in range(numTestMat):
        classResult = ClassIfy.classify(normMat[i, :], normMat[numTestMat:m, :], datingLabels[numTestMat:m], 3)
        ...
        if (classResult != datingLabels[i]):
            erroCount += 1.0
    print("这个算法的成功率为: %f" % (erroCount / (float)(numTestMat)))
