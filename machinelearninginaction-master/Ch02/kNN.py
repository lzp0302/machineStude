...
def datingClassTest2():
	hoRatio=0.5
	datingDataMat,datingLabels = file2matrix2('datingTestSet2.txt')       #load data setfrom file
	normMat,ranges,minVals=autoNorm(datingDataMat)
	m=normMat.shape[0]
	numTestMat=int(hoRatio*m)
	erroCount=0
	for i in range(numTestMat):
		classResult=classify0(normMat[i,:],normMat[numTestMat:m,:],datingLabels[numTestMat:m],3)
